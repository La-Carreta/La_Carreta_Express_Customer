    ...
    Stream<List<OrdenPedido>> getOrdersByFilterState() {
      if (filterOptionSelected == "Todos") {
        ...
        return ref
            ...
            .getOrdersByFilter(customer.id, filterOptionSelected);
      }
    }
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.data!.isEmpty) {
                      return NoDataFound(
                          ...
                          maximiunHeight: maximiunHeight * 0.9,
                          pathLottie: "assets/lottie/json/no-orders.json",
                          text: "No hay pedidos");
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        const urlIcon =
                            "https://res.cloudinary.com/dwexseytn/image/upload/v1708610624/La_Carreta_Express/Various_icons/entrega-de-comida_vnwcbn.png";
                        return pedidoList[index].estadoOrden ==
                                "Pedido realizado"
                            ? Slidable(
                                key: UniqueKey(),
                                endActionPane: ActionPane(
                                    ...
                                    children: [
                                      if (pedidoList[index].estadoOrden ==
                                          "Pedido realizado")
                                        SlidableAction(
                                          onPressed: (context) {
                                            ref
                                                .watch(ordenPedidoProvider
                                                    .notifier)
                                                .cancelOrderById(
                                                    pedidoList[index].id);
                                            //Mostrar un snackbar
                                            showCustomSnackbar(
                                                context: context,
                                                title:
                                                    "Orden #${pedidoList[index].numOrden} cancelada satisfactoriamente.");
                                          },
                                          icon: Icons.delete,
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Center(
                  child: IconButton(
                      onPressed: () =>
                          context.push("/seguimiento-pedido/${pedido.id}"),
        ...
        if (pedido.statusPago) const _BadgePayment(),
      ],
    );
  }
}
    ...
    builder: (BuildContext context) {
      return Consumer(
        builder: (context, ref, _) {
          final selected = ref.watch(filterOptionPlates);
          ...
          return RadioGroup<int>(
            ...
            onChanged: (value) {
              ...
              if (value < 0 || value >= statuses.length) return;
              final status = statuses[value];
              ref.read(filterOptionPlates.notifier).state = value;
              ref.read(filterOptionPedidoProvider.notifier).state = status;
              if (status == 'Todos') {
                ref.read(ordenPedidoProvider.notifier).getOrders(customer.id);
              } else {
                ref
                    .read(ordenPedidoProvider.notifier)
                    .getOrdersByFilter(customer.id, status);
              }
            },
            child: ClipRRect(
