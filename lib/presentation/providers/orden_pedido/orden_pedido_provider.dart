...
final ordenPedidoProvider =
    StateNotifierProvider<OrdenPedidoNotifier, List<OrdenPedido>>((ref) {
  final createOrder = ref.watch(ordenPedidoRepositoryProvider).createOrder;
  final fetchOrder =
      ref.watch(ordenPedidoRepositoryProvider).getOrdersByCustomer;
  final fetchOrderByFilter =
      ref.watch(ordenPedidoRepositoryProvider).getOrdersByFilter;
...
typedef OrdenPedidoCallBackByCustomer = Stream<List<OrdenPedido>> Function(
    {required String idCliente});
typedef OrdenPedidoCallBackByCustomerAndFilter = Stream<List<OrdenPedido>>
    Function({required String idCliente, required String state});
typedef OrderPedidoCallBackCreate = Future<void> Function(
    {required OrdenPedido order});
typedef OrderPedidoCallBackCancel = Future<void> Function(
...
class OrdenPedidoNotifier extends StateNotifier<List<OrdenPedido>> {
  ...
  Stream<List<OrdenPedido>> getOrdersByFilter(
      String idCliente, String orderState) {
    return fecthOrdersByCustomerAndFilter(
        idCliente: idCliente, state: orderState);
  }
